...
void toggleInteractive(std::string args) {
    g_interactiveMode = !g_interactiveMode;

    for (auto& bg : bgWindows) {
        if (auto bgw = bg.lock())
            bgw->m_hidden = !g_interactiveMode;
    }

    g_pInputManager->refocus();

    Log::logger->log(Log::INFO, "[hyprwinwrap] interactive mode: {}", g_interactiveMode ? "ON" : "OFF");
}
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    HyprlandAPI::addDispatcher(PHANDLE, "hyprwinwrap:toggle", toggleInteractive);
